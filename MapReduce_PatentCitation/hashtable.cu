						...
						int pagePerGroup)
{
	...
	mbk->availableGPUMemory = (740 * (1 << 20));
	...
	cudaMalloc((void**) &(mbk->depochSizes), epochNum * numThreads * sizeof(int));
	cudaMemset(mbk->depochSizes, 0, epochNum * numThreads * sizeof(int));
