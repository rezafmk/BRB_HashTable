...
__device__ inline int getUserAID(char* record)
{
	return myAtoi(&record[USERAIDLOCATION]);
}

__device__ inline int getUserBID(char* record)
{
	return myAtoi(&record[USERBIDLOCATION]);
}

__device__ inline int getUserARate(char* record)
{
	return myAtoi(&record[USERARATELOCATION]);
}
__device__ inline int getUserBRate(char* record)
{
	return myAtoi(&record[USERBRATELOCATION]);
}

__global__ void netflixKernelMultipass(
				char* data, 
				int numRecords,
				unsigned numUsers,
				ptr_t* textAddrs,
