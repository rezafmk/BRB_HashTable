				...
				)
{
	...
	for(int j = 0; reloop; j ++)
	{
		...
		if(!prediction && j > 1 && mbk->depochSuccessStatus[j - 2] == (char) SUCCEED)
		{
			i += mbk->depochSizes[(numThreads * 2) * (j - 2) + index + BLOCKSIZE];//epochDataSizePerThread;
			continue;
		}
#endif
		...
		if(!prediction && j > 1)
		{
			...
			int iCounter = 0;
			int iterations = 0;
			for(; (usedDataSize < epochDataSizePerThread) && (i < end); i ++, iterations ++)
			{
