...
__device__ bool addToHashtable(void* key, int keySize, void* value, int valueSize, multipassConfig_t* mbk)
{
	...
	do
	{
		...
		if(oldLock == 0)
		{
			...
			{
				void* oldValue = (void*) ((largeInt) existingBucket + sizeof(hashBucket_t) + keySize);
				resolveSameKeyAddition(key, value, oldValue);
			}
			else
			{
				hashBucket_t* newBucket = (hashBucket_t*) multipassMalloc(sizeof(hashBucket_t) + keySize + valueSize, group, mbk, groupNo);
				if(newBucket != NULL)
				{
					...
					for(int i = 0; i < (keySize / ALIGNMET); i ++)
					...
					for(int i = 0; i < (valueSize / ALIGNMET); i ++)
						*((largeInt*) ((largeInt) newBucket + sizeof(hashBucket_t) + keySize + i * ALIGNMET)) = *((largeInt*) ((largeInt) value + i * ALIGNMET));
				}
				else
				{
					success = false;
						...
						int pagePerGroup)
{
	...
	mbk->availableGPUMemory = (1000 * (1 << 20));
